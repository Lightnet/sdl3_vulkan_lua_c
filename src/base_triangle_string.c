...
// #include "triangle_frag_spv.h" // fragment shader header SPIR-V ( generate )
// #include "triangle_vert_spv.h" // vertex shader header SPIR-V ( generate )
...
VkShaderModule createShaderModule(VkDevice device, const char* code, shaderc_shader_kind kind) {
    shaderc_compiler_t compiler = shaderc_compiler_initialize();
    if (!compiler) {
        printf("Failed to initialize shaderc compiler\n");
    ...
    shaderc_compilation_result_t result = shaderc_compile_into_spv(
        ...
        strlen(code),
        kind,
        "shader.glsl", // Input file name (for error messages)
        "main",        // Entry point
    ...
    const char* spvCode = shaderc_result_get_bytes(result);

    VkShaderModuleCreateInfo createInfo = {VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO};
    createInfo.codeSize = codeSize;
    createInfo.pCode = (const uint32_t*)spvCode;
...
// Fragment shader (GLSL) - Fixed for C
const char* fragmentShaderSource = 
    ...
    // Create SDL window
    SDL_Window* window = SDL_CreateWindow("Vulkan Triangle Strings", 800, 600, SDL_WINDOW_VULKAN | SDL_WINDOW_RESIZABLE);
    if (!window) {
    ...
    VkShaderModule vertShaderModule = createShaderModule(device, vertexShaderSource, shaderc_vertex_shader);
    if (vertShaderModule == VK_NULL_HANDLE) {
        printf("Failed to create vertex shader module\n");
    }

    VkShaderModule fragShaderModule = createShaderModule(device, fragmentShaderSource, shaderc_fragment_shader);
