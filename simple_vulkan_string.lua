...
local window = sdl.create_window("SDL3 Vulkan Lua 5.4 Demo String", 800, 600, sdl.WINDOW_VULKAN | sdl.WINDOW_RESIZABLE)
local window_id = window.windowID

local appinfo = vulkan.create_vk_application_info({
    application_name = "SDL3 Vulkan Lua",
...
local vertShaderModule = vulkan.create_shader_module_str(device, vertexShaderSource, vulkan.shaderc_vertex_shader)
print("vertShaderModule:" .. tostring(vertShaderModule))
if not vertShaderModule then
    error("Failed to create vertex shader module")
end
local fragShaderModule = vulkan.create_shader_module_str(device, fragmentShaderSource, vulkan.shaderc_fragment_shader)
